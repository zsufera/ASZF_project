...
export function Badge({ kind, value }: BadgeProps) {
  ...
  if (kind === "priority" && String(value).toLowerCase().includes("sürg")) {
    return <span className={`${base} bg-status-urgent-bg text-status-urgent-fg`} aria-label={`Prioritás: ${value}`}>● {value}</span>;
  }
  if (kind === "escalation") {
    return <span className={`${base} bg-status-esc-bg text-status-esc-fg`} aria-label={`Eszkaláció: ${value}`}>⚠ {value}</span>;
  }
  if (kind === "confidence") {
  ...
  if (kind === "sla") {
    return <span className={`${base} bg-white border border-one-line text-one-grey`} aria-label={`SLA: ${value} nap`}>⏱ {value} nap</span>;
  }
  if (kind === "channel") {
