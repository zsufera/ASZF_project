  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-one-turq-l to-white border border-one-line rounded-one p-3 mb-4 flex flex-wrap items-center gap-2">
        <span className="font-bold text-[15px]">Ugy #{caseData.case_id}</span>
        <Badge kind="category" value={caseData.category_label} />
        {caseData.priority === "surgos" ? <Badge kind="priority" value="SURGOS" /> : null}
        {caseData.escalated ? <Badge kind="escalation" value="Eszkalalt" /> : null}
