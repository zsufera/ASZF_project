...
def test_app_renders_login_when_no_user():
    at = AppTest.from_file("legacy/ui/app.py").run()
...
def _sample_case():
    return {
        ...
        "inbound_text_masked": "TĂ©ves dĂ­jtĂ©telt talĂˇltam a szĂˇmlĂˇn.",
        "customer_candidates": [],
        "draft_versions": [],
        "agent_state": {
            "retrieval": {"chunks": [{"chunk_id": "c1", "paragrafus": "12.3",
                                       "quote": "a dĂ­j mĂłdosĂ­tĂˇsĂˇt 30 nappal elĹ‘re"}]},
            "policy_map": {},
            "timeline": [{"step": "classify", "output": {"category": "szamlazas"}}],
            "draft": {"subject": "VĂˇlasz", "body_masked": "Tisztelt ĂśgyfelĂĽnk!",
                      "citations": []},
            "escalation": {"required": True, "reasons": ["ismĂ©tlĹ‘dĹ‘ panasz"]},
        },
    }
...
def test_case_view_renders_with_timeline():
    def script():
        from unittest import mock
        import legacy.ui.views.case_view as cv
...
def test_inbox_view_renders_items():
    def script():
        from unittest import mock
        import legacy.ui.views.inbox_view as iv
        iv.api_client = mock.MagicMock()
        iv.api_client.ApiError = RuntimeError
        iv.api_client.list_inbox.return_value = {"items": [
            {"case_id": "1", "category_label": "SzĂˇmlĂˇzĂˇs", "priority": "surgos",
             "status_label": "Ăšj", "channel_label": "email", "subject": "TĂ©ves szĂˇmla",
...
def test_copilot_view_renders_empty_chat():
    def script():
        from unittest import mock
        import legacy.ui.views.copilot_view as cv
        cv.api_client = mock.MagicMock()
        cv.api_client.ApiError = RuntimeError
...
def test_copilot_stream_lines_yields_talking_points():
    import legacy.ui.views.copilot_view as cv
    out = "".join(cv._stream_lines("â€˘ pont 1\nâ€˘ pont 2"))
...
def test_copilot_create_case_button_returns_case_id():
    ...
    at = AppTest.from_function(script).run()
    assert not at.exception
    assert any("Ăśgy lĂ©trehozĂˇsa" in b.label for b in at.button)
...
def test_free_input_view_renders():
    def script():
        from unittest import mock
        import legacy.ui.views.free_input_view as fv
...
def test_postal_view_renders():
    def script():
        from unittest import mock
        import legacy.ui.views.postal_view as pv
...
def test_evaluation_view_renders():
    def script():
        from unittest import mock
        import streamlit as st
        import legacy.ui.views.evaluation_view as ev
...
def test_supervisor_view_renders():
    def script():
        from unittest import mock
        import legacy.ui.views.supervisor_view as sv
